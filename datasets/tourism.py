...
"""
Record Meta info of the dataset
...
class TourismDataset:
    ...
    def load(training: bool = True) -> 'TourismDataset':
        ...
        for group in TourismMeta.seasonal_patterns:
            ...
            """
            Transpose the df so that
            Each row in train is a time series
            Each row in test is a time series
            length of each time series is stored in meta_length
            """
            if group == 'Yearly':
                train_meta = train[:2]
            ...
            if training:
                ...
                dataset = test
            # Connect the values of each ts into [[],[],[]...]
